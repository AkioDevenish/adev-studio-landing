    ...
    >
      <div className="max-w-screen-xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start mb-32">
          ...
          >
            My{" "}
            <span className="italic text-foreground/60">learning journey</span>{" "}
        ...
        <div className="relative">
          ...
          >
            {phases[activePhase].items.map(
              (area: PhaseItem, index: number) => (
                <motion.div
                  key={index}
                  style={{ y: index % 2 === 0 ? y1 : y2 }}
                  className={`group border-t border-foreground/10 pt-8 ${
                    index % 2 !== 0 ? "md:mt-20" : ""
                  }`}
                >
                  <div className="flex justify-between items-baseline mb-6">
                    <span className="text-xs font-mono text-muted bg-foreground/5 px-2 py-1 rounded flex items-center gap-2">
                      ...
                      {activePhase === 2 &&
                        area.title ===
                          "Multisensory Specialization Thesis" && (
                          <div className="relative">
                            <div className="w-2 h-2 rounded-full bg-green-500" />
                            <div className="absolute inset-0 w-2 h-2 rounded-full bg-green-400 animate-ping" />
                          </div>
